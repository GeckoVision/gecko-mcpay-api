...
# Bundled prompt versions. v5.2 is the current default — Judge-only structural
# fix for v5.1 under-shipping ideas explicitly named in MANDATORY SHIP rules
# (see docs/prompts/v5_2-changelog.md). v5.1 was a Judge-only fix for the
# 2026-04-28 verdict_accuracy regression (see docs/prompts/v5_1-changelog.md).
...
# GECKO_PRO_PROMPTS_VERSION=v5.1 (or v5, v4) to pin a prior bundle without code
# changes.
...
def load_prompts() -> dict[str, str]:
    ...
    2. ``GECKO_PRO_PROMPTS_VERSION`` (``v4``, ``v5``, ``v5.1``, or ``v5.2``)
       — selects which bundled file to load. Default is ``v5.2`` (Judge
       structural fix: parallel SHIP/KILL rules → strict execution pipeline,
       so SHIP-by-name exits before the KILL check is reachable). ``v5.1``,
       ``v5``, and ``v4`` are rollback targets.
    3. Bundled default (``v5.2``).
    """
