...
async def lifespan(_: FastAPI) -> AsyncIterator[None]:
    ...
    try:
        ...
        if firewall_runner is not None:
            # The runner owns the shared ws client but doesn't start it — start it
            # here so both vault subs (runner) and init-log subs (discovery)
            # multiplex on one live connection.
            ws = firewall_runner.ws_client
            if hasattr(ws, "start"):
                await ws.start()
            await firewall_runner.start()
            app.state.safety_runner = firewall_runner
            # Pool discovery: the feed that finds new launch pools and registers
            # them with the runner. Without it the runner watches nothing.
            firewall_discovery = build_discovery(firewall_runner)
            if firewall_discovery is not None:
                await firewall_discovery.start()
                app.state.safety_discovery = firewall_discovery
                logger.info("launch_firewall.discovery_started")
...
# V1 verdict-loop — session-gated POST /v1/research. Runs the SAME basic trade
# panel as the public /trade_research surface, but behind require_session, for the
# logged-in web app's first-user validation loop (pass/defer + dissent +
# citations). X402_MODE stays stub; no real-payment wiring here.
from gecko_api.routes.research import router as _research_router  # noqa: E402
