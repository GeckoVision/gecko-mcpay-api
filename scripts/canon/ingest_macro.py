"""Ingest Fed / BIS / IMF macro working papers into the canon corpus.

13 curated public-domain PDFs (see ``gecko_core.sources.canon_macro``).
Chunks land with ``provider_kind="canon_macro"``, ``protocol=()``
(cross-cutting, Pattern F), ``freshness_tier="static"``,
``content_kind="mechanism"``. Idempotent via (source_id, chunk_index).

    set -a; source .env; set +a   # MONGODB_URI + OPENAI_API_KEY
    uv run python scripts/canon/ingest_macro.py --dry-run   # verify URLs
    uv run python scripts/canon/ingest_macro.py             # live ingest
"""
...
from gecko_core.sources.canon_macro import MACRO_SOURCES, CanonSource  # noqa: E402
from gecko_core.sources.pdf import extract as pdf_extract  # noqa: E402

log = logging.getLogger("canon.macro")

_SESSION = uuid5(NAMESPACE_URL, "gecko.canon.macro.session.v1")
CHUNK_SIZE = 1000  # macro working papers are longer-form analytical prose


def _slug(s: CanonSource) -> str:
    last = s.author.split(",")[0].split()[-1] if s.author else "unknown"
    venue_short = s.venue.split()[0] if s.venue else "?"
    return f"{venue_short}-{last}-{s.year}"


async def ingest_one(s: CanonSource, *, dry_run: bool) -> dict[str, int]:
    source_id = uuid5(NAMESPACE_URL, s.url)
    try:
        text, _ = await pdf_extract(s.url)
    except Exception as exc:  # noqa: BLE001
        log.warning("FAIL | %s | fetch_error: %s", s.url, exc)
        return {"chunks": 0, "skipped": 1}
    if not text:
        log.warning("FAIL | %s | empty extract (scanned PDF?)", s.url)
        return {"chunks": 0, "skipped": 1}
    chunks_text = chunk(text, size=CHUNK_SIZE)
    if not chunks_text:
        log.warning("FAIL | %s | zero chunks", s.url)
        return {"chunks": 0, "skipped": 1}
    log.info("FETCH %-26s bytes=%7d chunks=%3d title=%s",
             _slug(s), len(text), len(chunks_text), s.title[:48])
    if dry_run:
    ...
    inserted = await insert_chunks_mongo(
        ...
        provider_kind="canon_macro", source_url=s.url,
...
async def amain(*, dry_run: bool, limit: int | None, sleep_seconds: float) -> int:
    sources = list(MACRO_SOURCES)[: limit] if limit else list(MACRO_SOURCES)
    log.info("=== canon-macro: %d PDFs (dry_run=%s) ===", len(sources), dry_run)
    total_chunks = total_skipped = 0
    per_source: list[tuple[str, int, int]] = []
    for i, s in enumerate(sources, 1):
        log.info("[%d/%d] %s", i, len(sources), s.url)
        st = await ingest_one(s, dry_run=dry_run)
        total_chunks += st["chunks"]
        total_skipped += st["skipped"]
        per_source.append((s.url, st["chunks"], st["skipped"]))
        if i < len(sources):
            await asyncio.sleep(sleep_seconds)
    log.info("=== DONE: chunks=%d skipped=%d ===", total_chunks, total_skipped)
    for url, n, skipped in per_source:
        log.info("  %s | %s | chunks=%d", "FAIL" if skipped else "OK", url, n)
    return 0
...
def main(dry_run: bool, limit: int | None, sleep_seconds: float, verbose: bool) -> None:
    """Ingest Fed / BIS / IMF macro working papers."""
