...
rows = []
for a in artifacts:
    d = json.load(open(LR / a))
    for r in d["rows"]:
        rows.append({"src": a, "id": r["id"], **r["scores"]})
print(f"pooled rows: {len(rows)}")

DIMS = ["verdict_accuracy","citation_relevance","provider_kind_coverage",
        "hallucination_score","dissent_grounding","confidence_calibration"]
THR = {"verdict_accuracy":0.85,"citation_relevance":0.50,"provider_kind_coverage":0.70,
       "hallucination_score":0.30,"dissent_grounding":0.50,"confidence_calibration":0.55}
RESAMPLES = 10000
SEED = 4242
ALPHA = 0.05
...
for d in DIMS:
    ...
    hw=(hi-lo)/2
    locked = lo>=THR[d]
    n30[d]={"mean":m,"lo":lo,"hi":hi,"hw":hw,"thr":THR[d],"locked":locked,
            "sd":statistics.pstdev(vals)}
    print(f"  {d:<24} mean={m:.3f} CI=[{lo:.3f},{hi:.3f}] hw={hw:.3f} thr={THR[d]:.2f} sd={n30[d]['sd']:.3f}  {'LOCKED' if locked else 'NOT-LOCKED'}")
...
for d in DIMS:
    ...
    print(f"  {d:<24} {p_lock*100:>6.1f}%      {hw15:.3f}    {hw30:.3f}    {ts:<9} {'LOCKED' if locked30 else 'not'}")
...
out={"n30":{d:{k:round(v,4) if isinstance(v,float) else v for k,v in n30[d].items()} for d in DIMS},
     "green30":green30,"ship_gate_pass":len(green30)==6,
     "n15":{d:{"p_lock":round(lock15_count[d]/SUB_DRAWS,4),
               "hw15":round(statistics.mean(hw15_acc[d]),4),
               "hw30":round(n30[d]["hw"],4),
               "flip_risk":round(flip_risk[d],4)} for d in DIMS}}
